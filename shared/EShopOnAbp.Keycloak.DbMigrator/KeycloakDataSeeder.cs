...
public class KeyCloakDataSeeder : IDataSeedContributor, ITransientDependency
{
    ...
    private async Task CreateWebGatewaySwaggerClientAsync()
    {
        var swaggerClient =
            (await _keycloakClient.GetClientsAsync(_keycloakOptions.RealmName, clientId: "WebGateway_Swagger"))
            .FirstOrDefault();
        ...
        if (swaggerClient == null)
        {
            ...
            swaggerClient = new Client
            {
                ...
                Name = "Web Gateway Swagger Client Application",
                Protocol = "openid-connect",
                Enabled = true,
                RedirectUris = new List<string>
                {
